...
public class NiceString {
    ...
    private static char[] vowels = {'a','e','i','o','u'};//list of vowels
    private static String[] disallowed = {"ab","cd","pq","xy"};//list of disallowed combinations
    
    private static boolean isNice(String input){//check each of the 3 conditions for being nice
        if(!checkVowels(input)){//does it have at least 3 vowels?
            return false;
        }
        else if(!checkDouble(input)){//does it have double letters?
            return false;
        }
        else if(!checkDisallowed(input)){//does it contain disallowed doubles?
            return false;
        }
